...
|---|---|---|
| Input fails Zod validation | `ErrorCode.InvalidParams` | Convert `z.ZodError` directly to this |
| External service failure | `ErrorCode.InternalError` | API timeout, DB error, unexpected exception |
| Resource or entity not found | `ErrorCode.InvalidRequest` | Include the missing identifier in the message |
| Auth / permission denied | `ErrorCode.InvalidRequest` | Do not reveal why access was denied |
| Tool or method not implemented | `ErrorCode.MethodNotFound` | Should not occur if routing is correct |
