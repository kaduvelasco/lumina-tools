...
> **Arch Linux:** a instalação de VS Code, VSCodium e Windsurf requer um AUR helper (`yay` ou `paru`) previamente instalado. O LuminaDev não instala AUR helpers automaticamente.
...
Instala o **Kitty Terminal** via script oficial (`sw.kovidgoyal.net/kitty/installer.sh`) e o **Starship prompt**. Aplica configurações completas:

- Fonte **JetBrains Mono Nerd Font v3.3.0** — instalada automaticamente pelo script, independente da opção 1 do menu
- Scrollback de 10.000 linhas, suporte a layouts e múltiplas janelas/tabs
- Configuração de tema interativa via `ctrl+shift+F2` (kitten Themes)
- Starship prompt com seleção de preset interativa (gruvbox-rainbow, tokyo-night, pastel-powerline, pure-preset)
- Integração com o sistema: entrada `.desktop` e ícone registrados
- Opção interativa para definir como terminal padrão (via `update-alternatives` no apt, `xdg-settings` no dnf/pacman)
...
**Pós-instalação:** a configuração da `GOOGLE_API_KEY` é solicitada ao final da instalação. Gere sua chave em [aistudio.google.com/apikey](https://aistudio.google.com/apikey).
...
- **Desktop** via GitHub Releases — detecta automaticamente a versão mais recente e a arquitetura do sistema:
  - apt (Ubuntu/Debian): pacote `.deb` (amd64 / arm64)
  - dnf (Fedora): pacote `.rpm` (x86_64 / aarch64)
  - pacman (Arch): AppImage instalado em `~/.local/bin/opencode-desktop` (x86_64 e aarch64)
...
Instala o **Zed Editor** via script oficial (`zed.dev/install.sh`) e aplica configurações com JetBrains Mono, tema One Dark, assistant Claude pré-configurado e suporte a PHP/Moodle via `phpactor`.

> **Atenção:** o `phpactor` (language server PHP) deve ser instalado separadamente. O Zed solicitará sua instalação na primeira abertura de um arquivo PHP, ou instale manualmente via Extensions (`ctrl+shift+x`).
...
- **apt (Ubuntu/Debian):** adiciona chave GPG em `/etc/apt/keyrings/` e repositório em `/etc/apt/sources.list.d/windsurf.list`, instala via `apt-get`
- **dnf (Fedora):** importa chave RPM e cria `/etc/yum.repos.d/windsurf.repo`, instala via `dnf`
- **pacman (Arch/CachyOS):** instala via AUR (`windsurf`) com `yay` ou `paru`

Os repositórios são adicionados de forma idempotente — apenas se ainda não existirem. As configurações ficam em `~/.codeium/windsurf`.

---

## 🐧 Distros suportadas

| Distro        | Package Manager | Status         | Observações                                        |
| ------------- | --------------- | -------------- | -------------------------------------------------- |
...
**ShellCheck** — lint estático em todos os arquivos `.sh` com `--severity=warning`. A exclusão `SC1091` (source de arquivo externo) é aplicada globalmente; avisos específicos de variáveis exportadas (`SC2034`) são suprimidos inline com `# shellcheck disable` diretamente em `utils.sh`.
...
3. Siga o padrão dos scripts existentes: cabeçalho, `set -euo pipefail`, funções documentadas e uso de `utils.sh`
4. Certifique-se de que o ShellCheck passa sem warnings: `shellcheck --severity=warning --exclude=SC1091 seu-script.sh`
